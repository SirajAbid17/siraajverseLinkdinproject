...
export default function PostComponent({ searchQuery = "", onClearSearch }) {
  ...
  // Update filteredPosts only when postdata or searchQuery changes
  useEffect(() => {
    if (postdata && Array.isArray(postdata)) {
      const filtered = filterPosts(postdata, searchQuery);
      setFilteredPosts(filtered);
      
      // Only reset to first page if search query changes, not when postdata updates
      if (searchQuery.trim()) {
        setCurrentPage(1);
      }
    }
  ...
  const handleLike = async (postId) => {
    try {
      ...
      // Update the specific post without causing a complete re-render
      setpostdata((prev) => {
